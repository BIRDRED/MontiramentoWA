...
@ManagedBean
@SessionScoped
public class EparACOMBean implements Serializable {
	...
	public EparACOMBean() throws IOException, InterruptedException {
		 EparReport ert = new EparReport();
		 ReportResponse report =  ert.getEparReport("b2w-acom", date);
						 ...
						 for(ReportData rd : rdData.getBreakdown()){
							 if(rd.getCounts().get(0) > 0 || rd.getCounts().get(1) >0){
							 	if(rd.getName().equals("BUSCA ORGANICA")){
							 		...
							 		totalOutrosVisits = new BigDecimal(totalOutrosVisits.doubleValue() + rd.getCounts().get(0)); 
							 		totalOutrosOrders = new BigDecimal(totalOutrosOrders.doubleValue() + rd.getCounts().get(1));
							 	} 	
							 }
						 }
			...
			//Instancia dos dados e Adicionar na Lista
					if(totalAfiliadosOrders.doubleValue() > 0 && totalAfiliadosVisits.doubleValue() > 0){
					 ...
					 ReportResponse reportly =  ertly.getEparReport("b2w-acom", dately);
	...
	private void populateData() throws IOException, InterruptedException {
		EparACOMBean epar = new EparACOMBean();
	...
	public static void main(String[] args) throws IOException, InterruptedException {
		EparACOMBean eparb = new EparACOMBean();
