...
@ManagedBean
@SessionScoped
public class EparSHOPBean implements Serializable {
	...
	BigDecimal totalEuVinaTVVisits;
	BigDecimal totalEuVinaTVOrders;
	...
	BigDecimal totalEuVinaTVVisitsLY;
	BigDecimal totalEuVinaTVOrdersLY;
	...
	public EparSHOPBean() throws IOException, InterruptedException {
		 EparReport ert = new EparReport();
		 ReportResponse report =  ert.getEparReport("b2w-shop", date);
		 			...
		 			//Inicialização de Soma 
							...
							totalEuVinaTVVisits = BigDecimal.ZERO;
							totalEuVinaTVOrders  = BigDecimal.ZERO;
					...
					//Inicialização de Soma - Ultimo Ano
							...
							totalEuVinaTVVisitsLY = BigDecimal.ZERO;
							totalEuVinaTVOrdersLY  = BigDecimal.ZERO;
						...
						//Faz a soma dos Variaveis
						 for(ReportData rd : rdData.getBreakdown()){
							 if(rd.getCounts().get(0) > 0 || rd.getCounts().get(1) >0){
							 	...
							 	}else if(rd.getName().equals("EU VI NA TV")){
							 		totalEuVinaTVVisits = new BigDecimal(totalEuVinaTVVisits.doubleValue() + rd.getCounts().get(0)); 
							 		totalEuVinaTVOrders = new BigDecimal(totalEuVinaTVOrders.doubleValue() + rd.getCounts().get(1));
							 	}
							 }
						 }
			...
			//Instancia dos dados e Adicionar na Lista
					if(totalAfiliadosOrders.doubleValue() > 0 && totalAfiliadosVisits.doubleValue() > 0){
					 ...
					 if(totalEuVinaTVOrders.doubleValue() > 0 && totalEuVinaTVVisits.doubleValue() > 0){
						 	Epar eparEuvinaTV = new Epar("EU VI NA TV", "EU VI NA TV", totalEuVinaTVVisits, totalEuVinaTVOrders);
						 	eparlist.add(eparEuvinaTV);
					 }
					 ...
					 ReportResponse reportly =  ertly.getEparReport("b2w-shop", dately);
									...
									//Faz a soma dos Variaveis
									 for(ReportData rdly : rdDataLY.getBreakdown()){
										 if(rdly.getCounts().get(0) > 0 || rdly.getCounts().get(1) >0){
										 	...
										 	}else if(rdly.getName().equals("EU VI NA TV")){
										 		totalEuVinaTVVisitsLY = new BigDecimal(totalEuVinaTVVisitsLY.doubleValue() + rdly.getCounts().get(0)); 
										 		totalEuVinaTVOrdersLY = new BigDecimal(totalEuVinaTVOrdersLY.doubleValue() + rdly.getCounts().get(1));
										 	}
										 }
									 }
						 		}
					...
					//Instancia dos dados e Adicionar na Lista
						if(totalAfiliadosOrdersLY.doubleValue() > 0 && totalAfiliadosVisitsLY.doubleValue() > 0){
						 ...
						 if(totalSocialAdsOrdersLY.doubleValue() > 0 && totalSocialAdsVisitsLY.doubleValue() > 0){
							 	Epar eparSocialAds = new Epar("SOCIAL ADS", "SOCIAL ADS", totalSocialAdsVisitsLY, totalSocialAdsOrdersLY);
						 ...
						 if(totalEuVinaTVOrdersLY.doubleValue() > 0 && totalEuVinaTVVisitsLY.doubleValue() > 0){
							 	Epar eparEuvinaTV = new Epar("EU VI NA TV", "EU VI NA TV", totalEuVinaTVVisitsLY, totalEuVinaTVVisitsLY);
							 	eparlistLY.add(eparEuvinaTV);
						 }
	...
	public BigDecimal getTotalEuVinaTVVisits() {
		return totalEuVinaTVVisits;
	}

	public void setTotalEuVinaTVVisits(BigDecimal totalEuVinaTVVisits) {
		this.totalEuVinaTVVisits = totalEuVinaTVVisits;
	}

	public BigDecimal getTotalEuVinaTVOrders() {
		return totalEuVinaTVOrders;
	}

	public void setTotalEuVinaTVOrders(BigDecimal totalEuVinaTVOrders) {
		this.totalEuVinaTVOrders = totalEuVinaTVOrders;
	}

	public BigDecimal getTotalEuVinaTVVisitsLY() {
		return totalEuVinaTVVisitsLY;
	}

	public void setTotalEuVinaTVVisitsLY(BigDecimal totalEuVinaTVVisitsLY) {
		this.totalEuVinaTVVisitsLY = totalEuVinaTVVisitsLY;
	}

	public BigDecimal getTotalEuVinaTVOrdersLY() {
		return totalEuVinaTVOrdersLY;
	}

	public void setTotalEuVinaTVOrdersLY(BigDecimal totalEuVinaTVOrdersLY) {
		this.totalEuVinaTVOrdersLY = totalEuVinaTVOrdersLY;
	}

	private void populateData() throws IOException, InterruptedException {
		EparSHOPBean epar = new EparSHOPBean();
	...
	public static void main(String[] args) throws IOException, InterruptedException {
		EparSHOPBean eparb = new EparSHOPBean();
		System.out.println(eparb.getChartEpar());
		System.out.println(eparb.getTotalSocialAdsOrders());
		System.out.println(eparb.getTotalSocialAdsOrdersLY());
		System.out.println(eparb.getTotalSocialAdsVisits());
		System.out.println(eparb.getTotalSocialAdsVisitsLY());
	}
}
